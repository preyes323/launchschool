...
describe('Albums View', function() {
  beforeEach(function() {
    $('#albums').html('');
    this.view = new AlbumsView({ collection: new Albums(albums) });
  });
  ...
  it('renders albums', function() {
    this.view.render();
    const albums = $('#albums li');
    expect(albums.length).toEqual(3);
  });

  it('re-renders albums', function() {
    this.view.render();
    const currentAlbums = $('#albums').html();
    this.view.collection.first().set('title', 'new title');
    const newAlbums = $('#albums').html();
    expect(currentAlbums).not.toEqual(newAlbums);
  });
});
